...
static int aoe_dyndevs = AOE_DYNDEVS;	/* AOE_DYNDEVS set in Makefile */
...
minor_get_static(ulong *sysminor, ulong aoemaj, int aoemin)
{
	...
	if (aoemin >= NPERSHELF) {
		printk(KERN_ERR
			"aoe: e%ld.%d: %s %d slots per shelf\n",
			aoemaj, aoemin,
...
aoedev_by_aoeaddr(ulong maj, int min, int do_alloc)
{
	...
	if (!d)
		goto out;
	d->targets = aoe_kcalloc(NTARGETS, sizeof(*d->targets), GFP_ATOMIC);
	if (!d->targets) {
		kfree(d);
		d = NULL;
		goto out;
